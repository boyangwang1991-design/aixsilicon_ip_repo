...
module ut_pqc_kem_keygen;
  ...
  initial begin
    ...
    health=0;launch(1);repeat(5) @(negedge clk);check(error && !done,"unhealthy entropy fails closed");wipe();health=1;
    launch(1);repeat(8) @(negedge clk);check(busy && !done && !error,"pending dependency remains busy");wipe();
    launch(3);repeat(80) @(negedge clk);check(error && !done,"stalled dependency has bounded failure");
    repeat(3) @(negedge clk);check(error && !done,"failure never becomes success");wipe();
    if(errors==0) $display("UT_pqc_kem_keygen: PASS (errors=0)");else $display("UT_pqc_kem_keygen: FAIL (errors=%0d)",errors);
    $finish;
  end
