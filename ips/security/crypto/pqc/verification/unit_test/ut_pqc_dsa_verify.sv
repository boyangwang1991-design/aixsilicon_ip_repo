`timescale 1ns/1ps
module ut_pqc_dsa_verify;
  logic clk=0,rst_n=0,start=0,clear=0;logic[3:0] pset=0;
  logic busy,done,error;logic mem_req,poly_start,codec_start,sampler_start,hash_start,hash_in_valid,sampler_valid,message_dma_req;
  int errors=0;
  always #5 clk=~clk;
  pqc_dsa_verify #(.MAX_CYCLES(64)) dut(.clk(clk),.rst_n(rst_n),.start(start),.clear(clear),.pset(pset),.mem_ready('0),.mem_rdata('0),.message_addr('0),.message_bytes('0),.context_bytes('0),.message_dma_done('0),.message_dma_error('0),.poly_done('0),.codec_done('0),.sampler_done('0),.sampler_error('0),.sampler_ready('0),.hash_in_ready('0),.hash_out_valid('0),.hash_done('0),.hash_out_data('0),.busy(busy),.done(done),.error(error),.mem_req(mem_req),.poly_start(poly_start),.codec_start(codec_start),.sampler_start(sampler_start),.hash_start(hash_start),.hash_in_valid(hash_in_valid),.sampler_valid(sampler_valid),.message_dma_req(message_dma_req));
  ...
  task wipe();@(negedge clk);clear=1;#1;
    check(!done && !error && !(mem_req|poly_start|codec_start|sampler_start|hash_start|hash_in_valid|sampler_valid|message_dma_req),"clear must immediately mask requests and result");
  ...
  initial begin
    ...
    if(errors==0) $display("UT_pqc_dsa_verify: PASS (errors=0)");else $display("UT_pqc_dsa_verify: FAIL (errors=%0d)",errors);
    $finish;
  end
