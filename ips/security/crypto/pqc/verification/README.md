...
│   ├── pqc_env.sv               # 环境顶层组件
│   ├── pqc_env_cfg.sv           # 环境配置
│   ├── pqc_env_dec.sv           # 环境声明（枚举、参数）
│   ├── pqc_rm.sv                # 参考模型
│   ├── pqc_rm_cfg.sv            # 参考模型配置
│   ├── pqc_checker.sv           # 检查器/Scoreboard
│   ├── pqc_checker_cfg.sv       # 检查器配置
│   ├── pqc_dut_cfg.sv           # DUT 配置
│   ├── pqc_virtual_sequencer.sv # 虚拟序列器
│   ├── pqc_virtual_sequence.sv  # 虚拟序列
│   ├── pqc_fcov.sv              # 功能覆盖率事务
│   ├── pqc_env.list             # 环境编译文件列表
│   └── utils/
...
graph TD
    ...
    subgraph YY["YY 验证环境层"]
        ...
        pqc_rm
        pqc_checker
        pqc_virtual_sequence
        pqc_virtual_sequencer
        pqc_fcov
    end
    ...
    apb_interface_agent --> pqc_env
    pqc_env_cfg --> pqc_env
    pqc_rm --> pqc_env
    pqc_checker --> pqc_env
    pqc_virtual_sequencer --> pqc_env
...
#### 4.2.2 pqc_rm.sv - 参考模型

```systemverilog
class pqc_rm extends uvm_component;
  uvm_analysis_imp #(apb_xaction, pqc_rm) in_export;
...
#### 4.2.3 pqc_checker.sv - 检查器

```systemverilog
class pqc_checker extends uvm_component;
  uvm_analysis_imp_act #(apb_xaction, pqc_checker) act_export;
  uvm_analysis_imp_exp #(apb_xaction, pqc_checker) exp_export;
...
class tc_sanity extends tc_base;
  virtual task run_phase(uvm_phase phase);
    pqc_sanity_vseq vseq;
    phase.raise_objection(this);
    vseq = pqc_sanity_vseq::type_id::create("vseq");
...
```text
pqc_env.sv           -> dma_env.sv
pqc_env_cfg.sv       -> dma_env_cfg.sv
pqc_checker.sv       -> dma_checker.sv
pqc_rm.sv            -> dma_rm.sv
pqc_virtual_sequence.sv -> dma_virtual_sequence.sv
```
...
3. **替换 DUT 名称 (yy -> dma)**
   ```bash
   find . -name "pqc_*" -exec rename 's/pqc_/dma_/' {} \;
   find . -type f -name "*.sv" -exec sed -i 's/pqc_/dma_/g' {} \;
   ```
