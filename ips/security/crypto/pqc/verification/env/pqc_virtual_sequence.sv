...
class pqc_base_vseq extends uvm_sequence;

  `uvm_object_utils(pqc_base_vseq)
  `uvm_declare_p_sequencer(pqc_virtual_sequencer)

  extern function new(string name = "pqc_base_vseq");
...
function pqc_base_vseq::new(string name = "pqc_base_vseq");
  super.new(name);
endfunction

task pqc_base_vseq::apb_write(input bit [31:0] addr, input bit [31:0] data);
  apb_xaction tr;
...
task pqc_base_vseq::apb_read(input bit [31:0] addr, output bit [31:0] data);
  apb_xaction tr;
  tr = apb_xaction::type_id::create("tr");
...
/// @class pqc_sanity_vseq
/// @brief Sanity virtual sequence - basic functionality test
///        Override body() with DUT-specific stimulus
class pqc_sanity_vseq extends pqc_base_vseq;

  `uvm_object_utils(pqc_sanity_vseq)

  extern function new(string name = "pqc_sanity_vseq");
  extern virtual task body();

endclass

function pqc_sanity_vseq::new(string name = "pqc_sanity_vseq");
  super.new(name);
endfunction

task pqc_sanity_vseq::body();
