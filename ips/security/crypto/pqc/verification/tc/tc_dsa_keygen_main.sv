...
class tc_dsa_keygen_main extends tc_base;
  `uvm_component_utils(tc_dsa_keygen_main)
  virtual pqc_main_if bus;
  byte unsigned ek[0:2591],dk[0:4895],desc[0:127];
  ...
  int seen_pk[0:2591],seen_handle[0:3],seen_comp[0:31];
  bit observing,ack_sent,ack_seen,was_stalled;
  logic [32:0] stalled_word;
  string vectors;
  function new(string name="tc_dsa_keygen_main",uvm_component parent=null);super.new(name,parent);endfunction
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual pqc_main_if)::get(this,"","main_bus",bus)) `uvm_fatal("DSA_KEYGEN","missing bus")
    if(!$value$plusargs("DSA_KEYGEN_VECTORS=%s",vectors)) `uvm_fatal("DSA_KEYGEN","missing frozen vectors")
  endfunction
      ...
      if(observing) begin
        if(bus.km_custody_header_valid && bus.km_custody_header_ready) begin
          ...
          if(headers!=1 || bus.km_custody_handle!==bus.km_generated_handle ||
             ...
             bus.km_custody_domain!==8'ha3 || bus.km_custody_algo!==4'd2 ||
  ...
  task run_case(int n);
    ...
    case_id=n;pset=n/3+4;pk_len=pset==4?1312:pset==5?1952:2592;dk_len=pset==4?2560:pset==5?4032:4896;
    $readmemh($sformatf("%s/%0d_pk.hex",vectors,n),ek,0,pk_len-1);
    $readmemh($sformatf("%s/%0d_sk.hex",vectors,n),dk,0,dk_len-1);
    $readmemh($sformatf("%s/%0d_keygen_desc.hex",vectors,n),desc);
    @(negedge bus.clk);
    $readmemh($sformatf("%s/%0d_xi.hex",vectors,n),bus.entropy_bytes,0,31);
    ...
    bus.entropy_tag={4'd2,4'(pset)};bus.entropy_words=4;bus.entropy_enable=1;bus.response_delay=19+n*3;
    apb_write(PQC_REG_INTR_STATE,32'h1f);apb_write(PQC_REG_INTR_ENABLE,1);
    observing=1;
    apb_write(PQC_REG_COMMAND,32'h10000010+(32'(pset)<<8));
    ...
    do begin
      ...
      if(err || rd[3]) `uvm_fatal("DSA_KEYGEN_STATUS",$sformatf("case=%0d status=%h",n,rd))
      apb_read(PQC_REG_INTR_STATE,intr,err);
      if(err) `uvm_fatal("DSA_KEYGEN_STATUS","interrupt read failed")
      if(polls>4000) `uvm_fatal("DSA_KEYGEN_TIMEOUT","command did not retire")
    end while(!intr[0]);
    ...
    if(pk_count!=pk_len || handle_count!=4 || comp_count!=32 || custody_count!=dk_len || headers!=1 || !ack_seen || read_count!=128)
      `uvm_fatal("DSA_KEYGEN_COUNTS","missing or duplicate output/confirmation")
    ...
    `uvm_info("DSA_KEYGEN_CASE_PASS",$sformatf("case=%0d pset=%0d checked pk=%0d private=%0d handle=4 completion=32",n,pset,pk_count,custody_count),UVM_NONE)
  endtask
  task run_phase(uvm_phase phase);
    ...
    `uvm_info("DSA_KEYGEN_MAIN_PASS","nine KATs with custody stalls and wrong-transaction ACK rejection",UVM_NONE)
