...
/// NOTE: Replace 'uvm_sequence_item' with your transaction type (e.g., apb_xaction)
class apb_slave_driver extends uvm_driver #(uvm_sequence_item /* TODO: Replace with apb_xaction */);
  ...
  apb_slave_driver_cfg  cfg;  // Slave driver configuration handle
  virtual apb_interface bus; // Virtual interface handle
...
function void apb_slave_driver::build_phase(uvm_phase phase);
  ...
  if (!uvm_config_db #(apb_slave_driver_cfg)::get(this, "", "cfg", cfg)) begin
    cfg = apb_slave_driver_cfg::type_id::create("cfg");
  end
endfunction
...
task apb_slave_driver::run_phase(uvm_phase phase);
  ...
  // Monitor for valid transactions (PSEL && PENABLE in ACCESS phase) and respond
  forever begin
    @(bus.mon_cb);

    if (bus.mon_cb.psel === 1'b1 && bus.mon_cb.penable === 1'b1) begin
      slave_response();
    end
  end
endtask
...
task apb_slave_driver::reset();
  ...
  //   bus.slv_cb.rdata <= '0;
  //   bus.slv_cb.resp  <= APB_RESP_OKAY;

  wait(bus.rst_n === 1'b1);
endtask

/// @brief Slave response definition
task apb_slave_driver::slave_response();
  int unsigned delay;
