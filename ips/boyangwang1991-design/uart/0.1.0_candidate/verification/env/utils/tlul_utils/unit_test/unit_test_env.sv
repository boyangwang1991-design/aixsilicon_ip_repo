...
class unit_test_env extends uvm_env;
    ...
    tlul_interface_agent     agent;       // Agent 实例
    tlul_interface_agent_cfg agent_cfg;   // Agent 配置
...
function void unit_test_env::build_phase(uvm_phase phase);
    ...
    // 创建 Agent 配置
    agent_cfg = tlul_interface_agent_cfg::type_id::create("agent_cfg");
    
    // 从 config_db 获取虚拟接口
    if (!uvm_config_db#(virtual tlul_interface)::get(this, "", "vif", agent_cfg.vif)) begin
    ...
    // 传递配置给 agent
    uvm_config_db#(tlul_interface_agent_cfg)::set(this, "agent", "cfg", agent_cfg);
    
    // 创建 Agent
    agent = tlul_interface_agent::type_id::create("agent", this);
