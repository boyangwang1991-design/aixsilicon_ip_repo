// =============================================================================
// File Name   : tlul_interface_agent.sv
// Description : XX interface agent top-level component
//               Replace 'xx' with actual protocol name (e.g., apb, axi, irq)
// =============================================================================

`ifndef TLUL_INTERFACE_AGENT__SV
`define TLUL_INTERFACE_AGENT__SV

/// @class tlul_interface_agent
...
class tlul_interface_agent extends uvm_agent;
  ...
  tlul_interface_agent_cfg cfg;   // Agent configuration handle
  virtual tlul_interface     vif; // Virtual interface handle
  ...
  // Example declarations:
    tlul_sequencer    sqr;         // Sequencer handle for generating transactions
    tlul_driver       mst_drv;     // Master driver handle for active agent
    tlul_slave_driver slv_drv;     // Slave driver handle for active agent
    tlul_monitor      mon;         // Monitor handle for passive observation
    tlul_monitor_cov  cov;         // Coverage collector handle for functional coverage

  // =============================================================================
  // Analysis Port
  // =============================================================================
  // Purpose: Broadcast observed transactions to scoreboard/checker via TLM
  uvm_analysis_port #(tlul_xaction) ap;  // Analysis port for transaction broadcasting

  `uvm_component_utils_begin(tlul_interface_agent)
  ...
  extern function new(string name = "tlul_interface_agent", uvm_component parent = null);
...
/// @param name   Agent name string
/// @param parent Parent component handle
function tlul_interface_agent::new(string name = "tlul_interface_agent", uvm_component parent = null);
...
/// @param phase Current phase handle
function void tlul_interface_agent::build_phase(uvm_phase phase);
  ...
  if (!uvm_config_db #(tlul_interface_agent_cfg)::get(this, "", "cfg", cfg)) begin
    `uvm_info(get_type_name(), "No cfg found in config_db, creating default configuration", UVM_LOW)
    cfg = tlul_interface_agent_cfg::type_id::create("cfg", this);
  end
  ...
  // =============================================================================
  if (!uvm_config_db #(virtual tlul_interface)::get(this, "", "vif", vif)) begin
  ...
  if (cfg.active) begin
    sqr = tlul_sequencer::type_id::create("sqr", this);
  end

  // Create driver based on mode (master/slave)
  if (cfg.active) begin
    if (cfg.mode == TLUL_MASTER_MODE) begin
      mst_drv = tlul_driver::type_id::create("mst_drv", this);
    end else begin
      slv_drv = tlul_slave_driver::type_id::create("slv_drv", this);
    end
  end

  // Create monitor (always created for both active and passive agents)
  mon = tlul_monitor::type_id::create("mon", this);

  // Create coverage collector if enabled
  if (cfg.enable_cov) begin
    cov = tlul_monitor_cov::type_id::create("cov", this);
  end
  ...
  // =============================================================================
  if (cfg.active) begin
    uvm_config_db #(virtual tlul_interface)::set(this, "mst_drv", "vif", vif);
    uvm_config_db #(virtual tlul_interface)::set(this, "slv_drv", "vif", vif);
  end
  uvm_config_db #(virtual tlul_interface)::set(this, "mon", "vif", vif);

endfunction

/// @brief Connect phase definition
/// @param phase Current phase handle
function void tlul_interface_agent::connect_phase(uvm_phase phase);
