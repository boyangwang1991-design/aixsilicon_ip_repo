...
class unit_test_checker extends uvm_component;
    ...
    uvm_analysis_imp #(apb_xaction, unit_test_checker) item_export;
    ...
    extern virtual function void check_transaction(apb_xaction tr);
...
/// @brief 写入函数定义
/// @param tr 接收到的事务
function void unit_test_checker::write(apb_xaction tr);
    item_count++;
...
/// @brief 事务检查定义
/// @param tr 要检查的事务
function void unit_test_checker::check_transaction(apb_xaction tr);
    // 检查响应类型
    if (tr.resp == APB_RESP_ERROR) begin
