...
var isShowRatios = true;
function showHideRatios(element) {
  if (isShowRatios) {
    $("table").removeClass("no-filter").addClass("filter");
    isShowRatios = false;
    element.setAttribute("title", "Show Ratios");
    element.innerHTML = "Show Ratios";
    $(".ratioshead[colspan='2']").each(function () {
      $(this).attr("colspan", "1");
    });
  } else {
    $("table").removeClass("filter").addClass("no-filter");
    isShowRatios = true;
    element.setAttribute("title", "Hide Ratios");
    element.innerHTML = "Hide Ratios";
    $(".ratioshead[colspan='1']").each(function () {
      $(this).attr("colspan", "2");
    });
  }
}
