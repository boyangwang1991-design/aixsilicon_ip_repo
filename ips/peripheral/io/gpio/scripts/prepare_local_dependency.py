...
materialized=output/'rtl'/source.name
...
core={'name':'aixsilicon:cbb:parity_gen_check:0.1.0',
      ...
      'filesets':{'rtl':{'files':['rtl/'+source.name],'file_type':'systemVerilogSource'}},
...
record={'reason':'Original core lacks paramtype and has legacy provider/backend metadata; source RTL unchanged',
        ...
        'materialized':{'path':str(materialized.relative_to(root)), 'sha256':hashlib.sha256(materialized.read_bytes()).hexdigest()}}
