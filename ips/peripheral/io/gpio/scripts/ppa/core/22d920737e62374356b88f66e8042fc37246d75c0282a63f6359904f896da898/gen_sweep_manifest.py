...
def main() -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="additional workspace-relative resolved dependency")
    ...
    try:
        ...
        extra = list(args.input)
        if (root / "model/parameter_space.yaml").is_file():
            extra.append("model/parameter_space.yaml")
        paths = {p for directory in ("rtl", "regs", "constraints", "configs", "generator", "scripts")
                 for p in (root / directory).rglob("*") if p.is_file() and "__pycache__" not in p.parts}
        paths.update(root.glob("*.core"))
        paths.update(root / raw for raw in extra)
        if any(not p.resolve().is_relative_to(root) for p in paths):
            raise ValueError("PPA inputs must be workspace-relative; materialize resolved dependencies")
        manifest["extra_inputs"] = extra
        manifest["inputs"] = [{"path": p.relative_to(root).as_posix(), "sha256": hash_file(p)} for p in sorted(paths)]
        # Final identity includes resolved dependencies, generator and parameter-model bytes.
        manifest["run_id"] = hashlib.sha256(json.dumps(
            {k: v for k, v in manifest.items() if k != "run_id"}, sort_keys=True,
            separators=(",", ":"),
        ).encode()).hexdigest()
        output = args.output or Path("evidence/ppa") / manifest["run_id"] / "manifest.yaml"
        if output.exists() and yaml.safe_load(output.read_text()) != manifest:
            raise ValueError("refuse to overwrite a different PPA run manifest")
