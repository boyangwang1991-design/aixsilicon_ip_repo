...
class gpio_checker extends uvm_component;
  ...
  gpio_checker_cfg cfg;  ///< Checker configuration handle

  uvm_analysis_imp_act #(apb_xaction, gpio_checker) act_export;  ///< Actual data input
  uvm_analysis_imp_exp #(apb_xaction, gpio_checker) exp_export;  ///< Expected data input
  ...
  `uvm_component_utils_begin(gpio_checker)
  ...
  extern function new(string name = "gpio_checker", uvm_component parent = null);
...
/// @param name   Checker name string
/// @param parent Parent component handle
function gpio_checker::new(string name = "gpio_checker", uvm_component parent = null);
...
/// @param phase Current phase handle
function void gpio_checker::build_phase(uvm_phase phase);
  super.build_phase(phase);

  if (!uvm_config_db #(gpio_checker_cfg)::get(this, "", "cfg", cfg)) begin
    cfg = gpio_checker_cfg::type_id::create("cfg");
  end
endfunction

/// @brief Connect phase definition
/// @param phase Current phase handle
function void gpio_checker::connect_phase(uvm_phase phase);
...
/// @param tr Actual transaction from DUT
function void gpio_checker::write_act(apb_xaction tr);
...
/// @param tr Expected transaction from reference model
function void gpio_checker::write_exp(apb_xaction tr);
  apb_xaction tr_clone;
...
function void gpio_checker::compare();
  apb_xaction act;
  apb_xaction exp;
...
/// @param phase Current phase handle
function void gpio_checker::check_phase(uvm_phase phase);
