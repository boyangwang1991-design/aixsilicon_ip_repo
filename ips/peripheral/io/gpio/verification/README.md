...
├── env/                        # 验证环境目录
│   ├── gpio_env.sv               # 环境顶层组件
│   ├── gpio_env_cfg.sv           # 环境配置
│   ├── gpio_env_dec.sv           # 环境声明（枚举、参数）
│   ├── gpio_rm.sv                # 参考模型
│   ├── gpio_rm_cfg.sv            # 参考模型配置
│   ├── gpio_checker.sv           # 检查器/Scoreboard
│   ├── gpio_checker_cfg.sv       # 检查器配置
│   ├── gpio_dut_cfg.sv           # DUT 配置
│   ├── gpio_virtual_sequencer.sv # 虚拟序列器
│   ├── gpio_virtual_sequence.sv  # 虚拟序列
│   ├── gpio_fcov.sv              # 功能覆盖率事务
│   ├── gpio_env.list             # 环境编译文件列表
│   └── utils/
│       └── apb_utils/           # 协议 Agent 目录
│           ├── src/
│           │   ├── apb_dec.sv              # 协议声明（参数、枚举）
│           │   ├── apb_interface.sv        # 协议接口
│           │   ├── apb_xaction.sv          # 事务类
│           │   ├── apb_driver.sv           # 主驱动
│           │   ├── apb_driver_cfg.sv       # 主驱动配置
│           │   ├── apb_slave_driver.sv     # 从驱动
│           │   ├── apb_slave_driver_cfg.sv # 从驱动配置
│           │   ├── apb_monitor.sv          # 监控器
│           │   ├── apb_monitor_cfg.sv      # 监控器配置
│           │   ├── apb_monitor_cov.sv      # 监控器覆盖率
│           │   ├── apb_sequencer.sv        # 序列器
│           │   ├── apb_interface_agent.sv  # Agent 顶层
│           │   ├── apb_interface_agent_cfg.sv # Agent 配置
│           │   ├── apb_sequence_library.svp   # 序列库
│           │   └── apb_package.sv          # 协议包
│           ├── unit_test/        # Agent 单元测试
│           └── apb_interface_agent.list  # Agent 编译文件列表
├── tc/                         # 测试用例目录
│   ├── tc_base.sv              # 基础测试类
...
graph TD
    ...
    subgraph YY["YY 验证环境层"]
        ...
        gpio_rm
        gpio_checker
        gpio_virtual_sequence
        gpio_virtual_sequencer
        gpio_fcov
    end
    ...
    uvm_object --> apb_xaction
    uvm_driver --> apb_driver
    uvm_driver --> apb_slave_driver
    uvm_monitor --> apb_monitor
    uvm_agent --> apb_interface_agent
    uvm_sequencer --> apb_sequencer
    uvm_sequence --> apb_sequence
    uvm_subscriber --> apb_monitor_cov

    apb_interface_agent --> gpio_env
    gpio_env_cfg --> gpio_env
    gpio_rm --> gpio_env
    gpio_checker --> gpio_env
    gpio_virtual_sequencer --> gpio_env
...
### 4.1 env/utils/apb_utils - 协议 Agent

协议 Agent 提供完整的总线协议验证组件。

#### 4.1.1 apb_dec.sv - 协议声明
...
typedef enum bit [1:0] {
  APB_RESP_OKAY  = 2'b00,
  APB_RESP_ERROR = 2'b01
} apb_resp_e;
```

#### 4.1.2 apb_interface.sv - 协议接口
...
interface apb_interface(input logic clk, input logic rst_n);
  ...
  logic [APB_DATA_WIDTH-1:0] wdata, rdata;
...
#### 4.1.6 apb_interface_agent.sv - Agent 顶层
...
#### 4.2.1 gpio_env.sv - 环境顶层

```systemverilog
class gpio_env extends uvm_env;
  gpio_env_cfg cfg;
  apb_interface_agent apb_agent;
  gpio_rm rm;
  gpio_checker checker;
  gpio_virtual_sequencer v_sqr;
...
#### 4.2.2 gpio_rm.sv - 参考模型

```systemverilog
class gpio_rm extends uvm_component;
  uvm_analysis_imp #(apb_xaction, gpio_rm) in_export;
  uvm_analysis_port #(apb_xaction) exp_ap;
  
  // 接收输入事务，生成期望输出
  virtual function apb_xaction process_transaction(apb_xaction tr);
endclass
```

#### 4.2.3 gpio_checker.sv - 检查器

```systemverilog
class gpio_checker extends uvm_component;
  uvm_analysis_imp_act #(apb_xaction, gpio_checker) act_export;
  uvm_analysis_imp_exp #(apb_xaction, gpio_checker) exp_export;
...
class tc_base extends uvm_test;
  gpio_env env;
  gpio_env_cfg env_cfg;
  
  virtual function void configure_env();
    env_cfg.apb_agent_cfg.active = APB_AGENT_ACTIVE;
    env_cfg.apb_agent_cfg.mode   = APB_MASTER_MODE;
    env_cfg.enable_rm           = 1;
...
class tc_sanity extends tc_base;
  virtual task run_phase(uvm_phase phase);
    gpio_sanity_vseq vseq;
    phase.raise_objection(this);
    vseq = gpio_sanity_vseq::type_id::create("vseq");
...
module harness;
  logic clk, rst_n;
  apb_interface apb_if (.clk(clk), .rst_n(rst_n));
  ...
  initial begin
    uvm_config_db #(virtual apb_interface)::set(
      null, "uvm_test_top.env.apb_agent", "vif", apb_if
    );
    run_test();
  end
endmodule
...
-f ./env/utils/apb_utils/apb_interface_agent.list
-f ./env/gpio_env.list
-f ./tc/tc.list
...
```text
apb_driver.sv        -> apb_driver.sv
apb_xaction.sv       -> apb_xaction.sv
apb_interface_agent.sv -> apb_interface_agent.sv
apb_interface.sv     -> apb_interface.sv
apb_package.sv       -> apb_package.sv
```
...
```text
gpio_env.sv           -> dma_env.sv
gpio_env_cfg.sv       -> dma_env_cfg.sv
gpio_checker.sv       -> dma_checker.sv
gpio_rm.sv            -> dma_rm.sv
gpio_virtual_sequence.sv -> dma_virtual_sequence.sv
```
...
2. **替换协议名称 (xx -> apb)**
   ```bash
   find . -name "apb_*" -exec rename 's/apb_/apb_/' {} \;
   find . -type f -name "*.sv" -exec sed -i 's/apb_/apb_/g' {} \;
   ```

3. **替换 DUT 名称 (yy -> dma)**
   ```bash
   find . -name "gpio_*" -exec rename 's/gpio_/dma_/' {} \;
   find . -type f -name "*.sv" -exec sed -i 's/gpio_/dma_/g' {} \;
   ```
...
1. 在 `env/utils/apb_utils` 基础上创建新 Agent
2. 修改 `apb_dec.sv` 定义协议参数和枚举
3. 修改 `apb_interface.sv` 定义协议信号
4. 修改 `apb_xaction.sv` 定义事务字段
5. 实现 `apb_driver.sv` 的驱动逻辑
6. 实现 `apb_monitor.sv` 的监控逻辑
