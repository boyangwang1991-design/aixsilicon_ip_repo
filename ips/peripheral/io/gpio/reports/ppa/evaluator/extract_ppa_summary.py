...
def extract(
    ...
    slack_match = min(SLACK_PATTERN.finditer(timing_text), key=lambda m: float(m.group("value")), default=None)
    dynamic_match = DYNAMIC_PATTERN.search(power_text)
    leakage_match = LEAKAGE_PATTERN.search(power_text)
    dynamic = _scaled(dynamic_match, {"w": 1e6, "mw": 1e3, "uw": 1.0}) if dynamic_match else None
    leakage = _scaled(leakage_match, {"w": 1e9, "mw": 1e6, "uw": 1e3, "nw": 1.0}) if leakage_match else None
    if dynamic is None or leakage is None:
        # DC report_power -hierarchy: units are independent; use the named top row.
        number = r"[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?"
        design = re.search(r"^Design\s*:\s*(\S+)", power_text, re.M)
        du = re.search(r"Dynamic Power Units\s*=\s*(" + number + r")\s*(mW|uW|W)\b", power_text, re.I)
        lu = re.search(r"Leakage Power Units\s*=\s*(" + number + r")\s*(nW|uW|mW|W)\b", power_text, re.I)
        row = re.search(r"^" + re.escape(design[1]) + r"\s+(" + number + r")\s+(" + number + r")\s+(" + number + r")\s+(" + number + r")\s+100(?:\.0*)?\s*$", power_text, re.M) if design else None
        if row and du and lu:
            if dynamic is None:
                dynamic = (float(row[1]) + float(row[2])) * _scaled(du, {"w": 1e6, "mw": 1e3, "uw": 1.0})
            if leakage is None:
                leakage = float(row[3]) * _scaled(lu, {"w": 1e9, "mw": 1e6, "uw": 1e3, "nw": 1.0})
    ...
    # Select the arrival and clock group belonging to the worst reported slack.
    prefix = timing_text[:slack_match.start()] if slack_match else ""
    path_text = prefix[prefix.rfind("Startpoint:"):] if "Startpoint:" in prefix else prefix
    group_match = re.search(r"Path Group:\s*(\S+)", path_text)
    edges = re.findall(r"clock\s+(\S+)\s+\((?:rise|fall) edge\)\s+(-?[\d.]+)", path_text)
    observed_period = None
    if len(edges) >= 2 and edges[-1][0] == edges[0][0]:
        observed_period = float(edges[-1][1]) - float(edges[0][1])
    period_matches = observed_period is None or abs(observed_period - period_ns) < 1e-6
    critical_delay = period_ns - slack if slack is not None and period_matches else None
    fmax = 1000.0 / critical_delay if critical_delay and critical_delay > 0 else None
    arrival_match = ARRIVAL_PATTERN.search(path_text)
    activity = "saif" if re.search(r"\bSAIF\b", power_text, re.I) else "default_probability_estimate"
    return {
        "schema_version": "2.0",
        "schema": "ip-ppa-summary/1.0",
        "analysis_evaluator": {"path": _evidence_path(Path(__file__)), "sha256": _sha256(Path(__file__))},
        "run_id": manifest["run_id"],
        "manifest": {"path": _evidence_path(manifest_path), "sha256": _sha256(manifest_path)},
        "config_id": manifest["config_id"],
        "clock_freq_mhz": freq,
        "params": manifest.get("params", {}),
        "area_um2": float(area_match.group(1)) if area_match else None,
        "slack_ns": slack,
        "timing_group": group_match[1] if group_match else None,
        ...
        "fmax_mhz": fmax,
        "dyn_power_uW": dynamic,
        "leak_power_nW": leakage,
        "activity": activity,
        ...
        } | {"activity": activity, "evaluator_sha256": _sha256(Path(__file__))},
        "raw_reports": {
            "area": {"path": _evidence_path(area_path), "sha256": _sha256(area_path)},
            "timing": {"path": _evidence_path(timing_path), "sha256": _sha256(timing_path)},
            "power": {"path": _evidence_path(power_path), "sha256": _sha256(power_path)},
        },
    }
