...
params.update({n:dict(datatype='int',paramtype='vlogparam',default=d) for n,d in dict(AUTO_START_MASK=0,NO_STOP_MASK=-1,HARD_CFG_LOCK_MASK=0).items()})
...
filesets['pv']={'files':['build/parameter_adapter/watchdog_parameter_config.sv','verification/parameters/ut_watchdog_parameters.sv'],'file_type':'systemVerilogSource'}
targets['param_check']={'default_tool':'vcs','filesets':['pkg','generated','rtl','pv'],'toplevel':'ut_watchdog_parameters','tools':{'vcs':{'vcs_options':['-full64','-sverilog','-ntb_opts','uvm-1.2','-timescale=1ns/1ps'],'run_options':['-no_save']}}}
...
(p/'ip-package.yaml').write_text(yaml.safe_dump(dict(schema_version='2.0',vendor='aixsilicon',library='ip',name='watchdog',version='1.0.0',vlnv='aixsilicon:ip:watchdog:1.0.0',status='candidate')))
