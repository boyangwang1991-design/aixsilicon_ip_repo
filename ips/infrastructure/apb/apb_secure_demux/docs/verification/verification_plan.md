...
status: approved
verification_baseline: ASD-VPLAN-1.0.0-USER-APPROVED
END_VPLAN_META -->

## 范围与准入

G0/G1有用户批准，LLD/字段行为有本轮明确批准，G2已具备原生寄存器及接口探针证据。此前CSR探针仅是生成器接口检查，不能充当以下IP用例已通过的证据。当前VPLAN已获用户针对评审包的明确批准，见docs/reviews/vp0_approval.md。
