...
| 文档版本 | 1.0.0-reviewed |
| 状态 | 用户已批准 LRS 与 CR-001～CR-003；不表示 RTL 或系统/协议签核通过 |
| 日期 | 2026-09-11 |
| 实现类型 | Parameterized SystemVerilog IP |
| 接口 | APB4，32 bit 数据，可信 MASTERID 扩展 |
| 配置描述 | YAML 实例配置；SystemRDL 为寄存器结构事实源，派生软件头文件/RTL/RAL |
...
| 0x04C | FIFO_POP | WO | bit0=1 弹出头记录；已实现但空 FIFO 返回 CMD_INVALID；FIFO 深度为 0 返回 CSR_UNIMPLEMENTED |
...
COMMIT_STATUS.FAIL_REASON：0=无；1=掩码非法；2=全局锁；3=端口锁；4=完整性故障。多个条件同时出现按此列表中的检查顺序（掩码、全局锁、端口锁、完整性）报告；多个端口取最低编号。未通过管理授权/基本 CSR 检查的访问不改变 COMMIT_STATUS。此诊断字段与第 9.1 节的 CSR 主错误独立排序：例如锁定且掩码为空时，日志原因是 CFG_LOCKED，而 COMMIT_STATUS.FAIL_REASON=1。
...
| 寄存器描述 | SystemRDL 结构 SSOT，本文地址/字段/复位/访问权限一致；YAML 仅作实例配置 |
